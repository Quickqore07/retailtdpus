...
use Illuminate\Support\Facades\DB;
    ...
    class WeeklySummaryRecalculator
    {
        ...
        public static function run(array $options = []): int
        {
            ...
            $companyIds = $options['company_ids'] ?? [];
            if(empty($companyIds) || $companyIds[0] == ''){
                $companyIds = [];
            }
            ...
            ActivityLogService::log(
                ...
                $options,
                null,
                'Recalculated weekly summary for ' . $employeeId . ' from ' . $startDate . ' to ' . $endDate . ' for company ' . implode(',', $companyIds) . ' and workgroup ' . $workgroupId
            );
            $workgroupCompanyIds = $workgroupId
                ? Company::where('workgroup_id', $workgroupId)->pluck('id')->toArray()
                : null;

            return (int) DB::transaction(function () use ($employeeId, $startDate, $endDate, $companyIds, $clearExisting, $roleId, $workgroupId, $workgroupCompanyIds) {
                $totalRecords = 0;
                if ($clearExisting) {
                    ...
                    if ($workgroupCompanyIds) {
                        $query->whereIn('company_id', $workgroupCompanyIds);
                    }
                    $query->delete();
                }
                ...
                foreach ($eowGroups as $eow => $datesInPeriod) {
                    ...
                    $minWageByState = MinimumWage::latestItemsKeyedByState($eow);
                    ...
                    if ($companyIds && !empty($companyIds)) {
                        $weekEmployeeIdQuery->whereIn('company_id', $companyIds);
                    }
                    if ($employeeId) {
                    ...
                    foreach ($employeeIdChunks as $idChunk) {
                        $weekQuery = EmployeeHours::whereBetween('date', [$weekStart, $weekEnd])
                            ...
                            ->selectRaw('
                                ...
                                ')
                            ->with('employee', 'company.state');
                        $mwaQuery = MWA::query()
                            ->whereIn('employee_id', $idChunk->all())
                            ->where('eow', $eow);
                        if ($companyIds && !empty($companyIds)) {
                            $weekQuery->whereIn('company_id', $companyIds);
                            $mwaQuery->whereIn('company_id', $companyIds);
                        }
                        if ($employeeId) {
                        ...
                        $effectiveRates = self::loadEffectiveRatesForChunk($idChunk->all(), $eow);
                        ...
                        foreach ($weekData as $row) {
                            ...
                            $rateKey = $row->employee_id . '-' . $row->role_id . '-' . $row->company_id;
                            $rate = $effectiveRates[$rateKey] ?? null;
                            ...
                            $mwaKey = $row->employee_id . '-' . $row->company_id . '-' . $row->role_id . '-' . $eow;
                            $row->mwa_amount = $mwaData[$mwaKey]->amount ?? 0;
                            $weekCalculated = self::calculatePayrollForSummary($row, $rate);
                            $minWageData = self::calculateMinimumWage($row, $rate, $minWageByState, $weekCalculated);
        ...
        private static function calculatePayrollForSummary($row, $rate): array
        {
            ...
            if ($rate->rate_type == 'Payroll Regular') {
                ...
                $total_check_pay = $rate->pay_type == 'HR' ? $total_hours * ($rate->ten99_rate ?? $rate->rate) : 0;
        ...
        /**
         * Load the effective rate per employee/role/company for a given EOW (one query per chunk).
         *
         * @return array<string, EmployeeRates>
         */
        private static function loadEffectiveRatesForChunk(array $employeeIds, string $eow): array
        {
            if (empty($employeeIds)) {
                return [];
            }

            $rates = EmployeeRates::query()
                ->whereIn('employee_id', $employeeIds)
                ->where('effective_date', '<=', $eow)
                ->where(function ($q) use ($eow) {
                    $q->whereNull('till_date')
                        ->orWhere('till_date', '0000-00-00')
                        ->orWhere('till_date', '>=', $eow);
                })
            ...
            foreach ($rates as $rate) {
                $key = $rate->employee_id . '-' . $rate->role_id . '-' . $rate->company_id;
                if (!isset($effectiveRates[$key])) {
                    $effectiveRates[$key] = $rate;
                }
            }
        ...
        private static function calculateMinimumWage($row, $rate, Collection $minWageByState, array $weekCalculated): array
        {
            ...
            $minWage = $minWageByState->get($row->company->state_id);
            if ($minWage && isset($minWage->minimum_wage)) {
                ...
                if ($weekCalculated['payroll_methods'] < $result['min_wage_weekly']) {
                    $result['min_wage_due'] = $result['min_wage_weekly'] - $weekCalculated['payroll_methods'];
                }
            }
            return $result;
        }
