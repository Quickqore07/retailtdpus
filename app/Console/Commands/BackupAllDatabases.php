...
class BackupAllDatabases extends Command
{
    ...
    /**
     * Extra database names always included in the named company-group folder
     * (in addition to that group's companies.db_name values).
     *
     * @var array<string, list<string>>
     */
    /*local*/
    // protected array $groupExtraDatabases = [
    //     'quickqore' => ['pj'],
    //     'kyn' => [],
    // ];
    protected array $groupExtraDatabases = [
        'patel123_erpquickqore' => ['patel123_erpquickqore', 'quickqore_reporting'],
        'kyn_erp' => ['kyn_erp', 'kyn_reporting'],
    ];

    /**
     * Source databases that own a companies table with db_name.
     * Folder name => source database name.
     *
     * @var array<string, string>
     */
    /*local*/
    // protected array $companySourceDatabases = [
    //     'quickqore' => 'pj',
    //     'kyn' => 'kyn',
    // ];
    protected array $companySourceDatabases = [
        'patel123_erpquickqore' => 'patel123_erpquickqore',
        'kyn_erp' => 'kyn_erp',
    ];
    ...
    {
        $groupedFiles = [];
        foreach (array_keys($this->companySourceDatabases) as $folder) {
            $groupedFiles[$folder] = [];
        }
        ...
        foreach ($dumped as $file) {
            $folder = $this->backupFolderForDatabase($file['database']);

            if ($folder !== null) {
                $groupedFiles[$folder][] = $file;
            } else {
                $others[] = $file;
            }
        }
    ...
    protected function backupFolderForDatabase(string $database): ?string
    {
        foreach ($this->companyGroupDatabaseNames() as $folder => $names) {
            if (in_array($database, $names, true)) {
                return $folder;
            }
        }
     ...
     */
    protected function companyGroupDatabaseNames(): array
    {
        static $cache = null;
        ...
        foreach ($this->companySourceDatabases as $folder => $sourceDatabase) {
            $names = $this->listCompanyDbNames($sourceDatabase);
            $extras = $this->groupExtraDatabases[$folder] ?? [];
            $cache[$folder] = array_values(array_unique(array_merge($names, $extras)));
        }
     ...
     * @return list<string>
     */
    protected function listCompanyDbNames(string $sourceDatabase): array
    {
        try {
            $table = $this->quoteDatabaseIdentifier($sourceDatabase) . '.`companies`';
            $rows = DB::connection($this->backupConnectionName())
                ->select("SELECT db_name FROM {$table} WHERE db_name IS NOT NULL AND db_name != ''");
        } catch (\Throwable $e) {
            $this->warn("Could not read {$sourceDatabase}.companies.db_name: " . $e->getMessage());
