...
class TransferFileInAWS extends Command
{
     ...
     * @var string
     */
    protected $signature = 'change:key-name  {--offset=0 : Skip first N employees from source list}
    ...
    public function handle(): int
    {
        ...
        $employeeIds = Employee::where('workgroup_id', 2)->whereIn('id', $this->array)->pluck('employee_id')->toArray();

        try {
            $data = DB::table('onboarding_list_2 as ol')
            ->whereIn('ol.employeeId', $employeeIds)
        ...
        $employees = Employee::where('workgroup_id', 2)->whereIn('id', $this->array)->pluck('id', 'employee_id')->toArray();
