...
trait Filterable
{
    ...
    public function scopeForBu($query, $bu)
    {
        return $bu ? $query->where('bu_id', $bu) : $query;
    }
    ...
    public function scopeSearchc($query)
    {
        $this->validate(request()->all(), [
            'column' => 'required|in:' . $this->searchableColumns()
        ]);
    ...
    public function scopeMetrics($query, $params, $type, $column, $time_peroid)
    {
        ...
        if ($type == 'chart') {
            ...
            if ($time_peroid == 'yesterday') {
                ...
                while ($start->lte($end)) {
                    $dates = Arr::add($dates, $start->copy()->format('H'), 0);
                    $start->addHour(1);
                }
                ...
                while ($start->lte($end)) {
                    $dates = Arr::add($dates, $start->copy()->format('d'), 0);
                    $start->addDay(1);
                }
                ...
                while ($start->lte($end)) {
                    $dates = Arr::add($dates, $start->copy()->format('m'), 0);
                    $start->addMonth(1);
                }
    ...
    public function scopeFilter($query, $cols = null, $options = [])
    {
        ...
        if (request('search') && property_exists($this, 'searchableColumns') && is_array($this->searchableColumns ?? [])) {
            $response = $response->where(function ($query) {
                foreach ($this->searchableColumns ?? [] as $column) {
                    if (strpos($column, '.') !== false) {
                        [$relation, $relColumn] = explode('.', $column, 2);
                        $query->orWhereHas($relation, function ($q) use ($relColumn) {
                            $q->where($relColumn, 'like', '%' . request('search') . '%');
                        });
                    } elseif (strpos($column, '/') !== false) {
                        $list = explode('/', $column);
                        foreach ($list as $c) {
                            $query->orWhere($c, 'like', '%' . request('search') . '%');
                        }
                    } else {
        ...
        return $response->paginate(
            request('limit', config('flow.per_page')),
            $cols ?? ['*']
        );
    }
    ...
    protected function process($data, $query)
    {
        $this->validate(
            ...
            [
                ...
                'f.*.query_2'  => 'required_if:f.*.operator,between,not_between,between_date',

            ],
            [

                // Column
                'f.*.column.required' => 'Please select a filter column',
                'f.*.column.in'       => 'The selected filter column is invalid',

                // Operator
                'f.*.operator.required'      => 'Please select a filter operator',
                'f.*.operator.required_with' => 'Please select a filter operator',
                'f.*.operator.in'            => 'The selected filter operator is invalid',

                // Values
                'f.*.query_1.required_unless' =>
                'Please enter a filter value for the selected operator.',

                'f.*.query_2.required_if' =>
                'Please enter a second value for the selected range operator.',
            ...
            [
                ...
                'f.*.query_1'  => 'filter value',
                'f.*.query_2'  => 'second filter value',
            ]
        );
    ...
    protected function validate($data, $rules, $messages = [], $customAttributes = [])
    {
    ...
    {
        return implode(',', array_merge(($this->sortable ?? []), ['created_at']));
    }
