...
export function usePermission() {
    ...
    const can = (resource, action) => {
        ...
        const permission = permissions.value.find(p => p.name === resource)
    ...
    const canAccess = (resource) => {
        ...
        return permission.actions?.index === 1 || permission.actions?.update === 1
    }
    ...
    const getAllowedActions = (resource) => {
        ...
        if(authStore.user?.role?.name?.toLowerCase() === 'admin') {
            return ['index', 'create', 'update', 'delete', 'show','approve','review','bank_details', 'upload', 'opening-balance', 'update-opening-balance', 'delete-opening-balance','documents-show','documents-upload','paid','payment','cancel','left']
        }
        const permission = permissions.value.find(p => p.name === resource)
        if (!permission) {
            return []
 ...
 * Usage: v-permission="'user.create'" or v-permission="['user', 'create']"
 */
