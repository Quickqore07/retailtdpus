<template>
    <div class="purchase-report p-2 bg-gray-50 dark:bg-gray-900">
        <!-- Filter Panel -->
        <Panel class="mb-6">
            <div class="mb-6">
                        ...
                        <tr>
                            <td :colspan="(reportPayload.weeks?.length ?? 0) + 3">
                                <NoData
                                    ...
                                    message="There is no purchase data for the selected year. Please select a different year or ensure ideal cost data has been entered."
                                    size="sm"
                                    icon-size="lg"
                                    :show-action="false"
...
const applyFilters = async () => {
    try {
        loading.value = true
        const response = await useRequest('post', '/reports/ideal-cost/purchase', {
...
const exportReport = async () => {
 ...
 // Empty row
        ...
        XLSX.writeFile(wb, `Purchase_Report_${filters.value.year}.xlsx`)
