<template>
    <div class="cash-short-sales-report p-2 bg-gray-50 dark:bg-gray-900">
        <!-- Filter Panel -->
        <Panel class="mb-6">
            <div class="mb-6">
                <h3 class="!text-lg !md:text-2xl font-bold text-gray-900 dark:text-white mb-2">
                    Cash Short Sales Report
                </h3>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    View company-wise short/over amounts for each day of the selected month
                </p>
            </div>

            <!-- Year, Month, and User Selection -->
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                ...
                <div v-if="can('cash-short-sales-report', 'regional-director')">
                    <DynamicDropdown
                        v-model="filters.regionalDirector"
                        resource="users?role=Regional Director &workgroup_check=true"
                        display-name="name"
                ...
                <div v-if="can('cash-short-sales-report', 'area-manager')">
                    <DynamicDropdown
                        v-model="filters.areaManager"
                        resource="users?role=Area Manager&workgroup_check=true"
                        display-name="name"
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700" v-if="sortedReportData.length > 0">
                        <tr v-for="(item, index) in sortedReportData" :key="index" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <Td
                                ...
                                :key="date"
                                align="center"
                                :customClass="'!min-w-[2.5rem]' + (item['date_' + date].amount ? ' !font-bold ' : '')"
            ...
            <div class="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                <div class="text-sm text-gray-700 dark:text-gray-300">
                    <template v-if="reportData.length">
                        ...
                        <template v-if="filters.regionalDirector">
                            · Filtered by Regional Director: <span class="font-medium">{{ filters.regionalDirector?.name }}</span>
                        </template>
                        <template v-if="filters.areaManager">
                            · Filtered by Area Manager: <span class="font-medium">{{ filters.areaManager?.name }}</span>
                        </template>
                    </template>
                    <template v-else>
...
const applyFilters = async () => {
    try {
        ...
        const response = await useRequest('post', '/reports/cash/cash-short-sales', payload)
...
const exportReport = async () => {
    try {
        ...
        await import('xlsx').then((XLSX) => {
            ...
            exportData.push(['Cash Short Sales Report'])
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Cash Short Sales Report')

            const filename = `Cash_Short_Sales_Report_${monthName.value}_${filters.value.selectedYear}_${new Date().getTime()}.xlsx`
...
const handleDoubleClick = (item, date) => {
    if(item['date_' + date].daily_sale_id){
        window.open(`/data-entry/daily-sales/${item['date_' + date].daily_sale_id}`, '_blank')
    }
}
