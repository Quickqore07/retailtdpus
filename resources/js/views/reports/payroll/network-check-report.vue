        ...
        <Panel class="mb-6">
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    View and analyze network check data for the selected period
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <Button
                    v-if="canImportQuickqore"
                        icon-left="upload"
                        icon-size="sm"
                        variant="secondary"
                        size="sm"
                        @click="importInQuickqore"
                        :loading="importQuickqoreLoading"
                        :disabled="!payrollData.length || allCheckIds.length === 0"
                    >
                        Import in quickqore
                    </Button>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900 md:sticky top-0 !z-[101]">
                        <tr>
                            ...
                            <Th class="md:sticky md:left-19 bg-white dark:bg-gray-900 !z-[101]">
                            ...
                            <Th class="md:sticky md:left-60 bg-white dark:bg-gray-900 !z-[101]">
                            ...
                            <Th class="md:sticky md:left-100 bg-white dark:bg-gray-900 !z-[101]">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 !z-[100]" v-if="groupedPayrollData.length > 0">
                        <template v-for="(group, groupIndex) in groupedPayrollData" :key="groupIndex">
                            ...
                            <tr class="bg-blue-50 dark:bg-blue-900/20 border-t-2 border-blue-300 dark:border-blue-700">
                                <Td weight="bold" color="primary" class="md:sticky md:left-0 bg-blue-50 dark:bg-blue-900/20 !z-[100]">
                                ...
                                <Td weight="bold" color="primary" class="md:sticky md:left-19 bg-blue-50 dark:bg-blue-900/20 !z-[100]">{{ group.companyName }}</Td>
                                <Td weight="bold" color="primary" class="md:sticky md:left-60 bg-blue-50 dark:bg-blue-900/20 !z-[100]"></Td>
                                <Td weight="bold" color="primary" class="md:sticky md:left-100 bg-blue-50 dark:bg-blue-900/20 !z-[100]">{{ formatNumber(group.total_amount) }}</Td>
                                <Td colspan="5" weight="bold" color="primary"></Td>
                            </tr>
                            ...
                            <tr v-for="(item, index) in group.items" :key="`${groupIndex}-${index}`" class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                <Td class="md:sticky md:left-0 bg-white dark:bg-gray-900 !z-[100]">
                                    <div class="flex items-center gap-2">
                                ...
                                <Td class="md:sticky md:left-19 bg-white dark:bg-gray-900 !z-[100]">{{ item.company?.name || 'N/A' }}</Td>
                                <Td class="md:sticky md:left-60 bg-white dark:bg-gray-900 !z-[100]">
                                ...
                                <Td :class="'md:sticky md:left-100 bg-white dark:bg-gray-900 !z-[100]' + (item.is_1099 ? ' !text-red-600 !dark:text-red-400' : '')">{{ formatNumber(item.amount) }}</Td>
                                <Td>
                                    <div class="flex items-center gap-2">
                                        <DynamicDropdown
                                ...
                                <Td>
                                    <p class="!mb-0" v-if="item.do_reviewed">DO {{ formatDateTime(item.do_reviewed_at) }}<template v-if="item.do_reviewed_by_name"> by {{ item.do_reviewed_by_name }}</template></p>
                                    <p class="!mb-0" v-if="item.hr_reviewed">HR {{ formatDateTime(item.hr_reviewed_at) }}<template v-if="item.hr_reviewed_by_name"> by {{ item.hr_reviewed_by_name }}</template></p>
                                    <p class="!mb-0" v-if="item.admin_reviewed">Admin {{ formatDateTime(item.admin_reviewed_at) }}<template v-if="item.admin_reviewed_by_name"> by {{ item.admin_reviewed_by_name }}</template></p>
                                </Td>
                                <Td>
                                    <Button
...
const canPrint = computed(() => {
    return can('check-printing-(paychex)', 'print')
})
const canReview = computed(() => {
    return can('check-printing-(paychex)', 'review')
})
const canUpdate = computed(() => {
    return can('check-printing-(paychex)', 'update')
})
const canImportQuickqore = computed(() => {
    return can('check-printing-(paychex)', 'import-quickqore')
})
...
const allCheckIds = computed(() => {
    return payrollData.value.map(item => item.check_id).filter(Boolean)
})
...
const importQuickqoreLoading = ref(false)
...
const makeDisabled = (item) => {
    ...
    return (item.reviewed ) || !canUpdate.value || new Date(item.check_date) < new Date()
}
...
const applyFilters = async () => {
    try {
        loading.value = true;
        selectAll.value = false;
        const response = await useRequest('post', '/reports/payroll/network-check', {
            ...
            payrollData.value.forEach((item, index) => {
                if(item.reviewed){
                    exportData.push([
                        ...
                        item.check_number,
                        item.check_date ? formatDate(item.check_date) : '-'
                    ])
                }else{     
                    exportData.push([
                        index + 1,
...
const reviewCheck = async () => {
    try {
        const count = payrollData.value.filter(item => (item.checked || item.reviewed) && item.amount > 0 && item.employee && item.company && item.ledger && item.fromCompany && item.check_number > 0).length;
...
const confirmPrint = async () => {
    try {
        ...
        window.open(response.zippath, '_blank')
        closePrintModal()
...
const importInQuickqore = async () => {
    try {
        if (allCheckIds.value.length === 0) {
            message.warning('No check IDs to import')
            return
        }
        importQuickqoreLoading.value = true
        await useRequest('post', '/reports/payroll/import-quickqore', {
            ids: allCheckIds.value,
        })
        message.success('Imported in quickqore successfully')
    } catch (error) {
        console.log(error)
        message.error(error.response?.data?.message || 'Failed to import in quickqore')
    } finally {
        importQuickqoreLoading.value = false
    }
}
