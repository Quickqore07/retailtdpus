<template>
    <div class="network-monthly-sales-report p-2 bg-gray-50 dark:bg-gray-900">
        <!-- Filter Panel -->
        <Panel class="mb-6">
            <div class="mb-6">
                <h3 class="!text-lg !md:text-2xl font-bold text-gray-900 dark:text-white mb-2">
                    Network Monthly Sales Report
                </h3>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    View company-wise total sales for each month of the selected year
                </p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs">
                    <thead class="bg-gray-100 dark:bg-gray-800 md:sticky top-0 !z-[101]">
                        <tr>
                            ...
                            <Th v-for="monthKey in months" :key="monthKey" class="text-center">
                                <div class="flex items-center gap-2 cursor-pointer select-none hover:text-blue-600 dark:hover:text-blue-400 transition-colors justify-center" @click="handleSort('pp_' + monthKey)">
                                    <span>{{ formatMonthLabel(monthKey) }}</span>
                                    <SvgIcon
                                        v-if="getSortDirection('pp_' + monthKey)"
                                        :name="getSortDirection('pp_' + monthKey) === 'asc' ? 'chevron-up' : 'chevron-down'"
                                        size="sm"
                                        class="text-blue-600 dark:text-blue-400"
                                    />
                                </div>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700" v-if="sortedSalesData.length > 0">
                        <tr v-for="(item, index) in sortedSalesData" :key="index" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            >
                                {{ formatNumber(item['pp_' + monthKey], 2) }}
                            </Td>
                    ...
                    <tbody class="bg-white dark:bg-gray-800" v-else>
                        <tr>
                            <td :colspan="months.length + 3" class="p-8">
                                <NoData
                                    icon="files"
                                    icon-color="blue"
                    ...
                    <tfoot class="bg-gray-100 dark:bg-gray-800 font-semibold" v-if="salesData.length > 0">
                        <tr>
                            ...
                            <Td v-for="monthKey in months" :key="monthKey" weight="bold" align="center">
                                {{ formatNumber(getColumnTotal(monthKey), 2) }}
                            </Td>
...
const getRowTotal = (item) => {
    let total = 0
    months.value.forEach(monthKey => {
        total += parseFloat(item['pp_' + monthKey] ?? 0)
    })
    return total
}
...
const applyFilters = async () => {
    try {
        loading.value = true
        const response = await useRequest('post', '/reports/sales/network-monthly-sales', {
...
const exportReport = async () => {
    try {
        ...
        await import('xlsx').then((XLSX) => {
            ...
            exportData.push(['Network Monthly Sales Report'])
            ...
            months.value.forEach(monthKey => {
                headers.push(formatMonthLabel(monthKey))
            })
            headers.push('Total')
            ...
            salesData.value.forEach((item) => {
                ...
                months.value.forEach(monthKey => {
                    row.push(parseFloat(item['pp_' + monthKey] || 0))
                })
            ...
            months.value.forEach(monthKey => {
                totalsRow.push(parseFloat(getColumnTotal(monthKey) || 0))
            })
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Network Monthly Sales')

            const filename = `Network_Monthly_Sales_Report_${filters.value.selectedYear}_${new Date().getTime()}.xlsx`
