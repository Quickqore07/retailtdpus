<template>
    <div v-if="show" class="area-form">
        <Panel :divider="true">
            <template #header>
                <div class="flex items-center justify-between">
                    <h5 class="font-bold">
                        {{ mode === 'edit' ? 'Edit Area' : 'Create New Area' }}
                    </h5>
                </div>
            </template>
...
const resource = route.meta?.resource || 'settings/areas'

const { form, errors, isSaving, show, mode, save, cancel, setData, access } = useFormable(resource, 'settings/areas', 'area')

watch(() => form.value.region, (newRegion) => {
    if (newRegion) {
        form.value.region_id = newRegion.id
        if (newRegion.state) {
            form.value.state = newRegion.state
            form.value.state_id = newRegion.state.id
        } else {
...
watch(() => form.value.state, (newState) => {
    if (newState) {
        form.value.state_id = newState.id
    } else {
        form.value.state_id = null
