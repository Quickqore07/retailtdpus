<template>
  <div v-if="show" class="bank-category-rule-form">
    <Panel :divider="true">
      <template #header>
        <div class="flex items-center justify-between">
          <h5 class="font-bold !m-0">
            {{ mode === 'edit' ? 'Edit Bank Category Rule' : 'Create Bank Category Rule' }}
          </h5>
        </div>
      </template>
      ...
      <form @submit.prevent="handleSave" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            placeholder="e.g., Credit Card Payment, Bank Transfer, etc."
            :required="true"
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span class="text-sm text-gray-600 dark:text-gray-400">When a transaction description meets</span>
          <select
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">of these conditions</span>
        </div>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3">
                  <Input
                    v-model="row.value"
                    placeholder="e.g., BANKCARD, TRANSFER, etc."
          ...
          <div class="flex items-start gap-2">
            ...
            <div class="text-sm text-blue-800 dark:text-blue-300">
              ...
              <ul class="list-disc list-inside space-y-1">
                <li><strong>Category Label:</strong> The label that will be assigned to matching transactions</li>
                <li><strong>Conditions:</strong> Define patterns to match in transaction descriptions</li>
                <li><strong>All:</strong> Transaction must match ALL conditions to be categorized</li>
                <li><strong>Some:</strong> Transaction needs to match ANY ONE condition to be categorized</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {{ mode === 'edit' ? 'Update Bank Category Rule' : 'Create Bank Category Rule' }}
          </Button>
        </div>
      </form>
    </Panel>
...
const ensureConditions = () => {
  if (!form.value?.conditions?.length) {
    form.value.conditions = [{ condition_type: 'Contains', value: '' }]
  }
}
...
const addCondition = () => {
  ...
  form.value.conditions.push({ condition_type: 'Contains', value: '' })
}
...
const handleSave = () => {
  const payload = {
    ...
    conditions: (form.value.conditions || []).map((c) => ({
      condition_type: c.condition_type || 'Contains',
      value: c.value ?? ''
    }))
  }
  save(payload)
