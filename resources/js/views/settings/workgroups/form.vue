<template>
    <div v-if="show" class="workgroup-form">
        <Panel :divider="true">
            <template #header>
                <div class="flex items-center justify-between">
                    <h5 class="font-bold !mb-0">
                        {{ mode === 'edit' ? 'Edit Workgroup' : 'Create New Workgroup' }}
                    </h5>
                </div>
            </template>
...
const resource = route.meta?.resource || 'settings/workgroups'

const { form, errors, isSaving, show, mode, save, cancel, setData, access } = useFormable(resource, 'settings/workgroups', 'workgroup')
