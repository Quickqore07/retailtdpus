<template>
    <div v-if="show" class="county-form">
        <Panel :divider="true">
            <template #header>
                <div class="flex items-center justify-between">
                    <h5 class="font-bold">
                        {{ mode === 'edit' ? 'Edit County' : 'Create New County' }}
                    </h5>
                </div>
            </template>
            ...
            <form @submit.prevent="save" class="space-y-4">
                ...
                <div class="flex items-center justify-end gap-3 pt-4">
                    ...
                    <Button variant="primary" size="md" type="submit" :loading="isSaving" 
                        ...
                        {{ mode === 'edit' ? 'Update County' : 'Create County' }}
                    </Button>
                </div>
            </form>
        </Panel>
...
const resource = route.meta?.resource || 'settings/counties'

const { form, errors, isSaving, show, mode, save, cancel, setData, access } = useFormable(resource, 'settings/counties', 'county')
