<template>
    <div v-if="show" class="region-form">
        <Panel :divider="true">
            <template #header>
                <div class="flex items-center justify-between">
                    <h5 class="font-bold">
                        {{ mode === 'edit' ? 'Edit Region' : 'Create New Region' }}
                    </h5>
                </div>
            </template>
...
const resource = route.meta?.resource || 'settings/regions'

const { form, errors, isSaving, show, mode, save, cancel, setData, access } = useFormable(resource, 'settings/regions', 'region')
