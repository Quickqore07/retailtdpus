<template>
  <div v-if="show" class="expense-type-form">
    <Panel :divider="true">
      <template #header>
        <div class="flex items-center justify-between">
          <h5 class="font-bold !mb-0">
            {{ mode === 'edit' ? 'Edit Expense Type' : 'Create Expense Type' }}
          </h5>
        </div>
      </template>
      ...
      <form @submit.prevent="save" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Input
            ...
            :required="true"
            :error="errors.amount_label ? errors.amount_label[0] : null"
          />
          <Input
            ...
            :required="true"
            :error="errors.other_amount_label ? errors.other_amount_label[0] : null"
          />
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
          ...
          >
            {{ mode === 'edit' ? 'Update Expense Type' : 'Create Expense Type' }}
          </Button>
        </div>
      </form>
    </Panel>
...
const { form, errors, isSaving, show, mode, save, cancel, setData, access } = useFormable(resource, 'ap/expense-types', 'expense-type')
