<template>
  <div v-if="show" class="shortage-show">
    <Panel :divider="true">
      <template #header>
        <div class="flex items-center justify-between">
          <h5 class="text-2xl font-bold !mb-0">Sales Shortages</h5>
          <div class="flex items-center gap-2">
            <Button icon-left="arrow-left" icon-size="sm" variant="secondary" size="xs" to="/data-entry/shortages" />
            <Button icon-left="edit" icon-size="sm" variant="primary" size="xs" v-if="access.includes('update')" :to="`/data-entry/shortages/${model.daily_sale?.id}/edit`" />
          </div>
        </div>
      </template>

      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-6">
        ...
        <Label label="Total Deposits" :value="`$${formatNumber(model?.daily_sale?.bank_deposit_total)}`" />
...
const resource = route.meta?.resource || 'data-entry/shortages'
const { model, show, setData,access  } = useShowable(resource, 'shortage')
