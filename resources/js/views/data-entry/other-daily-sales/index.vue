<template>
  <div class="other-daily-sales-index">
    <Filterable
      ref="filterableRef"
      title="Daily Sales"
      url="data-entry/other-daily-sales"
    ...
    >
      <template #extra>
        <Button
          ...
          variant="primary"
          size="sm"
          to="/data-entry/other-daily-sales/create"
        >
          New Daily Sale
        </Button>
      </template>
      ...
      <template #default="{ item, index }">
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <Td weight="medium" color="primary">${{ formatAmount(item.total_deposits) }}</Td>
          <Td weight="medium" color="primary">${{ formatAmount(getDifference(item)) }}</Td>
          ...
          <Td align="right" weight="medium">
            <div class="flex items-center justify-end gap-2">
              <router-link
                ...
                :to="`/data-entry/other-daily-sales/${item.id}`"
              ...
              <router-link
                ...
                :to="`/data-entry/other-daily-sales/${item.id}/edit`"
...
const resource = 'data-entry/other-daily-sales'
const { filterableRef, setData, removeDB, access } = useIndexable(resource, 'other-daily-sales')
...
const getDifference = (item) => {
  return Number(item?.cash_due || 0) - Number(item?.total_deposits || 0)
}
